...
Variable_names <- c("SubjectId", feature_names$V2, "ActivityId")
...
subset_data$ActivityId <- Activity_names$V2[match(subset_data$ActivityId, Activity_names$V1)]
subset_data <- rename(subset_data, Activity= ActivityId)

## 4.------------------------------------------------------------------------------------------------## 
# Descriptive variable names for each observation

colnames(subset_data) <- gsub("tBodyAcc|tBodyBodyAcc", "BodyAccTime", colnames(subset_data))
colnames(subset_data) <- gsub("fBodyAcc|fBodyBodyAcc", "BodyAccFreq", colnames(subset_data))
colnames(subset_data) <- gsub("tGravityAcc", "GravityAccTime", colnames(subset_data))
colnames(subset_data) <- gsub("tBodyGyro", "BodyAngVelTime", colnames(subset_data))
colnames(subset_data) <- gsub("fBodyGyro|fBodyBodyGyro", "BodyAngVelFreq", colnames(subset_data))
...
## 5.------------------------------------------------------------------------------------------------## 
# Calculate aggregate for each variable by Subject Id and Activity Id. Here we again use the library dplyr.

Population_mean<-summarise_each(group_by(subset_data, Activity, SubjectId),funs(mean)) 

## 6.------------------------------------------------------------------------------------------------## 
# writes the file to a text file in the current directory

write.table(Population_mean, file = "SamsungTidyDataset.txt", sep = "\t", row.names = FALSE)
